...
This document outlines the comprehensive testing strategy for the GlassMCP server project, covering unit tests, integration tests, security tests, and end-to-end testing scenarios.
...
export function setupWindowsAPIMocks(): MockWindowsAPI {
  ...
  mockUser32.FindWindowW.mockImplementation((className, windowName) => {
    const classStr = className ? Buffer.from(className, 'utf16le').toString() : null;
    const nameStr = windowName ? Buffer.from(windowName, 'utf16le').toString() : null;
...
import { setupWindowsAPIMocks, MockWindowsAPI } from '../../../tests/mocks/windows-api';
...
describe('WindowPlugin', () => {
  ...
  describe('focusWindow', () => {
    it('should focus window by exact title match', async () => {
      ...
      // Act
      const result = await plugin.focusWindow({ title: 'Calculator', exact: true });
      
      // Assert
      expect(result.success).toBe(true);
      expect(result.windowHandle).toBe(expectedHandle);
      expect(mockUser32.SetForegroundWindow).toHaveBeenCalledWith(expectedHandle);
    });
    ...
    it('should focus window by partial title match', async () => {
      ...
      // Act
      const result = await plugin.focusWindow({ title: 'Visual Studio', exact: false });
...
describe('API Integration Tests', () => {
  ...
  describe('WebSocket API', () => {
    it('should handle window commands via WebSocket', async (done) => {
      // Arrange
      const ws = new WebSocket(`ws://localhost:${app.server.address().port}/ws`);
...
describe('CLI E2E Tests', () => {
  it('should start server and handle commands', async () => {
    ...
    const serverProcess = spawn('node', ['dist/cli/index.js', 'start', '--port', '7701'], {
      stdio: 'pipe'
    });
  ...
  it('should handle playground mode', async () => {
    const playgroundProcess = spawn('node', [
      'dist/cli/index.js',
      'playground',
      '--non-interactive'
    ], { stdio: 'pipe' });
...
This comprehensive testing strategy ensures that GlassMCP is thoroughly tested across all layers, from individual functions to complete user workflows, with special attention to security and performance requirements.
